...
function output_youtube_live( $atts ) {
	...
	$script_path = $plugin_url . 'js/wp-youtube-live.js';
	$script_path_style = $plugin_url . 'css/wp-youtube-live.css'; 
	$script_path_api = 'https://www.youtube.com/iframe_api'; 
	
	// enqueue assets.
	wp_enqueue_script( 'wp-youtube-live', $script_path, array(), time(), true);
	wp_enqueue_style( 'wp-youtube-live', $script_path_style, array(), time(), true);
	wp_enqueue_script( 'youtube-iframe-api', $script_path_api, array(), time(), true);
...
function get_youtube_live_content( $request_options ) {
	...
	$True_Width        = wp_youtube_live_is_ajax() && array_key_exists( 'width', $_POST )
		? sanitize_key( wp_unslash( $_POST['width'] ) )
		: sanitize_key( $request_options['width'] );
	$True_Height       = wp_youtube_live_is_ajax() && array_key_exists( 'height', $_POST )
		? sanitize_key( wp_unslash( $_POST['height'] ) )
		: sanitize_key( $request_options['height'] );
	$youtube_live                     = new EmbedYoutubeLiveStreaming( esc_attr( $youtube_options['youtube_live_channel_id'] ), esc_attr( $youtube_options['youtube_live_api_key'] ), true, $True_Width, $True_Height);
	...
	if (!isset($error_message)) {
		$error_message = 'error is not set';
	}
	if ( 'no_message' !== $youtube_options['fallback_behavior'] ) {
